...
由规范 1（组件开发时独立）和规范 6（组件隔离、代码隔离）可知，任意两个组件之间是绝对解耦的，且可以通过 ARouter 和 EventBus 进行通信。

如果舍弃 EventBus 的通信方式（因为 EventBus 需要实例化对象），只保留 ARouter 的通信方式，就可以实现组件的社区化。

人们可以自由分享组件，通过选择别人写好的组件，套个壳编译一下，就是一个新的 APP！极大节约头发！

一个组件发布为 AAR 后，如何在另一个自己写的组件里通过 ARouter 来配置这个组件？

用门户模式和建造者模式。

对 AAR 来说，它要有个门户 Activity，这个 Activity 是视图上隐形的，用户看不见，而 Activity 有良好的生命周期控制，请在生命周期内完成组件配置。
AAR 需要提供一个配置表，里面是组件所需的配置信息。比如一个登录组件，需要 host，port，登陆 url，注册 url，找回密码 url，发送短信回调所需 secretKey 等等。
使用者用 ARouter 传递参数来初始化配置表即可。
```java
ARouter.getInstance()
    .withInt()
    .withString()
    .with...()
    .navigation(settingCallback)
```
优雅的链式调用。
